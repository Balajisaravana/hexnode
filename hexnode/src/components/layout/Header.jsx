...
const Header = () => {
  ...
  return (
    <nav className={`fixed group flex justify-center items-center w-full h-[69px] z-50 bg-blue hover:bg-white transition-colors duration-300 ${scrolled ? 'bg-white border border-white-tab' : 'bg-blue'}`} onMouseEnter={() => setScrolled(true)} onMouseLeave={() => setScrolled(false)}>
      <div className='container grid grid-cols-12'>
        <a className='col-span-5 my-auto' rel="nofollow noopener noreferrer" href="https://www.hexnode.com/" target='_blank'>
        ...
        <div className='col-span-7 text-end flex items-center justify-end'>
          <Button>
            <a role="link" aria-label="Start a 14-day free trial" href="https://www.hexnode.com/mobile-device-management/cloud/signup/" className={`action-btn font-medium text-[15px] leading-[24px] text-white! ${"lg:block hidden"}  `}>
              14 day free trial
            </a>
          </Button>
          <div className={`ml-2 ${scrolled ? "text-blue" : "text-white"} block lg:hidden group-hover:text-blue`} onClick={toggleSidebar}>
            ...
            >
              <rect x="3" y="6" width="18" height="2" fill={scrolled ? "black" : "white"} />
              <rect x="3" y="11" width="18" height="2" fill={scrolled ? "black" : "white"} />
              <rect x="3" y="16" width="18" height="2" fill={scrolled ? "black" : "white"} />
            </svg>
